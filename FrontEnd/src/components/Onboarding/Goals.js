...
export const Goals = () => {
  ...
  const validateGoals = () => {
    const errors = goals.map((goal) => {
      const error = {};
      if (!goal.goal_name)
        error.goal_name = "Please enter a name for your goal";
      if (goal.goal_type_id === 0)
        error.goal_type_id = "Please select a category";
      if (!goal.target_date)
        error.target_date = "Please enter a target date for your goal";
      if (
